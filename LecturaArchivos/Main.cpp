...
// DECLARACIONES
const int minimoAdulto = 18; // Solo utilice const para definir la variable
const string nombre = "Pedrosky Rey del Trosky"; // VARIABLE CONSTANTE
vector<string> listaNombres;


// funciones que si o si tienen que ir aqui arriba
...
void lecturaArchivos()
{
    ...
    while (getline(archivo, linea)) 
    {
        ...
        while (getline(ss, nombre, ',')) {
            listaNombres.push_back(nombre); // AQUI ESTAMOS guardando el nombre, le hacemos un push a la lista
        }
    }
    archivo.close();
}